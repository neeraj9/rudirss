...
TEST(TestAtomFeed, TestLoadSampleFeedWithMoreAttributes)
{
    ASSERT_TRUE(SUCCEEDED(CoInitializeEx(nullptr, COINIT_MULTITHREADED)));

    {
        try
        {
            ...
            AtomFeed atomFeed;
            atomFeed.ParseFromFile(std::format(L"{}\\..\\..\\testdata\\reddir_emacs.rss", dir));
            atomFeed.IterateFeeds([](const FeedData& feed) -> bool {
                wprintf(L"title: %s\nlink: %s\ndescription: %s\n\n",
                feed.GetValue(L"title").c_str(), feed.GetValue(L"link").c_str(), feed.GetValue(L"content").c_str());
                return true;
            });
            ASSERT_TRUE(true);
        }
